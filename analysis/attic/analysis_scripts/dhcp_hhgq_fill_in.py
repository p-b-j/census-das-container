...
if __name__ == "__main__":
    ...
    mb.add(
        desc = "Detailed Query, Keep Sparse",
        metric_function = lambda sdf: sdf.getGeolevels(geolevels).getQueryAnswers(queries)
    )
    mb.add(
        desc = "Detailed Query, Fill in Missing Rows",
        metric_function = lambda sdf: sdf.getGeolevels(geolevels).getQueryAnswers(queries).fill(queries)
    )
