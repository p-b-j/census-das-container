...
# 4/14/2020, lecle notes: switched this from copy/paste interactive code to a script that can be run stand-alone

# 5/12/2020, lecle notes: to help J. Woodell translate this to sparkSQL for use in TRR, went through these checks in detail, making notes in comments about how they work and what they compute. Also made notes where spec versions etc were unclear or appear dated.
...
dfper_s3_path = "s3://uscb-decennial-ite-das/users/lecle301/cnstatDdpSchema_DataIndUserSpecifiedQueriesNPass_National_dpQueries1_officialCNSTATrun/data-run1.0-epsilon4.0-DHCP_MDF.txt"
# Old example: "s3://uscb-decennial-ite-das/users/lecle301/DemonstrationProducts_Sept2019_fixedTotals_BAK2/full_person_unpickled/td4/run_0000/persons.csv"

dfunits_s3_path = "s3://uscb-decennial-ite-das/users/lecle301/cnstatDdpSchema_SinglePassRegular_nat_H1_Only_withMeasurements_v8/MDF_UNIT-run1.0-epsilon0.5-H1.csv"
...
if RUN_DF_PER_CHECKS:
    ...
    #   #MAFID  geocode hhgq
    # where hhgq should match hhgq in http://dasexperimental.ite.ti.census.gov/mediawiki/index.php/Tables_Needed(to_be_extracted_from_CEF)#Table_10:_PL94_Housing_File (actually, full schema should match; link is to schema for original Table10, it seems)
    cefu = spark.read.csv("s3://uscb-decennial-ite-das/title13_input_data/table10/", header=True, sep='\\t')
    ...
    # Loads Person MDF; for schema, see https://github.ti.census.gov/CB-DAS/etl_2020/tree/master/mdf
...
if RUN_DF_UNITS_CHECKS:
    ...
    # Loads the Units table extract from the CEF, with schema http://dasexperimental.ite.ti.census.gov/mediawiki/index.php/Tables_Needed(to_be_extracted_from_CEF)#Table_10:_PL94_Housing_File
    ...
    cefu = spark.read.csv("s3://uscb-decennial-ite-das/title13_input_data/table10_20190610/", sep="\t", header=True)
    ...
    # Reads MDF Households file; spec as given in https://github.ti.census.gov/CB-DAS/etl_2020/tree/master/mdf (possibly not most recent spec; depends on input MDF)
