...
if __name__ == "__main__":
    ...
    # the analysis object contains the spark session and save location path for this instance
    ...
    queries = ['detailed', 'numraces', 'cenrace', 'hhgq', 'hispanic', 'votingage', 'total', 'hhgq * votingage']
    
    # a metric builder object is needed for analysis, as experiments are built using it
    mb = sdftools.MetricBuilder()
    mb.add(
        desc = "L1 Sums of queries for all geolevels for each run",
        metric_function = lambda sdf: sdf.getGeolevels(geolevels).getQueryAnswers(queries).L1().show().sum(groupby=[AC.GEOCODE, AC.GEOLEVEL, AC.PLB, AC.QUERY, AC.RUN_ID]).show()
    )
    
    
    # this script is creating the DASRun objects first, in order to grab only a few of them,
    # before passing them into the analysis.add_experiment function.
    # Note that add_experiment's 'run' parameter can accept a list of either s3 paths or DASRun objects
    runpaths = []
    for path in experiment_paths:
        # only want the first two runs of each budget group for this experiment
        runpaths += treetools.getDASRuns(path)[0:2]

    analysis.add_experiment(
        name = "PL94_P12_Analysis",
        runs = runpaths,
        schema = schema_name,
        metric_builder = mb,
        mtype = AC.SPARSE
