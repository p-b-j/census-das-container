...
# --dynamic_allocation - run dynamic allocaion
#
...
do
   ...
   --pdf_only) PDF_ONLY='--pdf_only certificate.pdf' ;;
   --dynamic_allocation) DYNAMIC_ALLOCATION=true ;;
...
if [ -z ${INITIAL_EXECUTORS+x} ]; then
   INITIAL_EXECUTORS=$(( $NUM_EXECUTORS  ))
fi

if [ -z ${MIN_EXECUTORS+x} ]; then
   MIN_EXECUTORS=$(( $EXECUTORS_PER_NODE ))
fi
...
if [ x$SPARK_RESOURCES = x ]; then
 ...
 --conf spark.dynamicAllocation.maxExecutors=$MAX_EXECUTORS"
    ...
    echo INITIAL_EXECUTORS: $INITIAL_EXECUTORS
    echo MIN_EXECUTORS: $MIN_EXECUTORS
...
spark_submit="$SPARK_SUBMIT $SPARK_RESOURCES
    ...
    --conf spark.dynamicAllocation.shuffleTracking.enabled=$DYNAMIC_ALLOCATION
