...
class TestEngines:
    ...
    def getEngine(self, engine, setup_instance, use_spark, das_stub):
        engine_s_config = """
                  ...
                  geolevel_budget_prop: 1/3, 1/3, 1/3
    ...
    def test_engines(self, spark, use_spark, engine, dd_das_stub):
        ...
        private_nodes = engine_instance.run(block_nodes)
        assert private_nodes[0].count() == len(block_nodes_input)
        assert np.sum(list(map(lambda node: node.getDenseSyn(), private_nodes[0].collect()))) == np.sum(list(map(lambda node: np.sum(node.getDenseRaw()), block_nodes_input)))
