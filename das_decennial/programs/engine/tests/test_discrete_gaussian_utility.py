...
def test_limit_denominator(num, denom, limit):
...
def test_limit_denominator_random_cases(high):
...
def test_limit_denominator_upper_lower(num, denom, limit):
...
def test_limit_denominator_errors():
...
def test_RationalSimpleDiscreteGaussian_errors(sigma_sq, size):
...
def test_RationalScalarDiscreteLaplacen_errors(s, t):
    with pytest.raises(AssertionError):
...
@pytest.mark.parametrize("size", [2.0, [], 'sdf', {}, 0])
def test_RationalSimpleBernoulliExp_errors(gamma, size):
...
def test_floorsqrt_errors(num, denom):
    with pytest.raises(AssertionError):
        floorsqrt(num, denom)
