...
# ---
def SimpleDiscreteGaussian(variance=1.0, size=1, rng=None):
    return np.fromiter((ScalarDiscreteGaussian(variance=variance, rng=rng) for _ in range(size)), dtype=int)


def ScalarDiscreteGaussian(variance=1.0, rng=None):
...
def SimpleBernoulliExp(gamma=0.0, size=1, rng=None):
...
def ScalarBernoulliExp(gamma=0.0, rng=None) -> int:
    if 0. <= gamma <= 1.:
        k: int = 1
        a: bool = True
        while a:
...
def VectorDiscreteUniform(low=0, high=100, size=1, rng=None):
...
def Bernoulli(numer=1.0, denom=1.0, rng=None):
...
def VectorBernoulli(numer=1.0, denom=1.0, size=1, rng=None):
    # This function isolates use of the rng Bernoulli
    # (This for-testing-only implementation trusts the rng for Bernoulli r.v.'s)
...
# ---
def DiscreteLaplace(s=1, t=1, size=1, rng=None):
...
def BernoulliExp(gamma: Union[float, np.ndarray]=0.0, size=1, rng=None):
    ...
    if np.all(gamma >= 0.) and np.all(gamma <= 1.):
        ...
        A = np.ones_like(gamma)
        Amask = np.ones_like(gamma, dtype=bool)  # All indices are active
        num_active = gamma.size
        while num_active > 0:
            new_samples = np.array([VectorBernoulli(numer=gamma[i], denom=K[i], size=1, rng=rng)[0] for i in range(num_active)])
            num_active -= int(np.sum(1 - new_samples))
            A[Amask] = new_samples
            Amask = Amask * (A == 1)  # Indices at which A==0 become inactive
            K = K + Amask  # Increment K for active indices
