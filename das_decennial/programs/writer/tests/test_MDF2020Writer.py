...
from programs.nodes.nodes import GeounitNode, SYN, GEOCODE, INVAR
...
"""
    ...
    def makeNode(self, hholds, units, geocode='0'):
        ...
        node = GeounitNode(raw=hhold_hist, raw_housing=unit_hist, invar=invar, cons=cons, geocode_dict={1: 'Stub'}, geocode=geocode )
        node.syn = node.raw
    ...
    def test_makeHistRowsFromMultiSparseRecode(self, hholds, units):
        node = self.makeNode(hholds, units)
        rows = makeHistRowsFromMultiSparse(node.toDict(keep_attrs=(SYN, GEOCODE, INVAR)), self.schema, row_recoder=Household2010ToMDFUnit2020Recoder)
...
"""
    ...
    def test_makeHistRowsFromMultiSparseRecode(self, persons):
        node = self.makeNode(persons)
        rows = makeHistRowsFromMultiSparse(node.toDict(keep_attrs=(SYN, GEOCODE, INVAR)), self.schema, row_recoder=DHCPHHGQToMDFPersons2020Recoder)
        ## TODO: Some testing of the MDF spec output should probably be done here. Maybe on just one case. Maybe not, and just test it within the
        #   writer test below
        assert len(rows) == len(persons)
