...
def log_demo(val):
    ...
    subprocess.call("logger -n 10.252.44.226 -p local1.info  bar".split(" "))

    env.get_census_env()
    master_ip = os.environ['MASTER_IP']

    subprocess.call(f"logger -n 10.252.44.226 -p local1.info master_ip={master_ip} added_syslog={clogging.added_syslog}".split(" "))
