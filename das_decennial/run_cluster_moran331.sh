...
### zip file is now created and shipped in the setup module (programs/das_setup.py)

### Create the ZIP file of all the code. This will be sent to the nodes with the spark-submit.
###
...
MEM_OVERHEAD=10g
MAX_EXECUTORS=120
SPARK_RESOURCES="--driver-memory $DRIVER_MEMORY --num-executors $NUM_EXECUTORS --executor-memory $EXECUTOR_MEMORY --executor-cores $EXECUTOR_CORES --driver-cores $DRIVER_CORES --conf spark.dynamicAllocation.maxExecutors=$MAX_EXECUTORS --conf spark.driver.maxResultSize=0g --conf spark.executor.memoryOverhead=$MEM_OVERHEAD"
...
cmd="spark-submit $SPARK_RESOURCES --conf spark.local.dir=/mnt/tmp/ --conf spark.eventLog.enabled=true --conf spark.eventLog.dir=/mnt/tmp/logs/  --master yarn --conf spark.submit.deployMode=client --conf spark.network.timeout=3000 das2020_driver.py $config --loglevel DEBUG"
echo $cmd
